...
public class KillAura extends Module {
    ...
    /**
     * hurtTime value from the previous isHitSelectPaused() call.
     * Used to detect the exact tick hurtTime transitions from >0 to 0,
     * which is the optimal moment to open the next burst window.
     */
     ...
     *
     * State machine:
     *   IDLE  → waiting for hurtTime→0 transition to open first burst
     *   BURST → APS-driven click attempts, one per 50ms tick
     *   PAUSE → all clicks blocked for exactly pauseDurationMs (wall clock)
     *
     * Burst trigger:
     *   prevHurtTime > 0 && currentHurtTime == 0
     *   The exact tick the server marks the entity hittable again.
     *   Opening the burst here maximises the chance clicks land on a valid
     *   server hit window rather than into leftover invulnerability frames.
     *
     * Pause contract:
     *   if (currentTime - lastBurstEnd < pauseDurationMs) → cancel click
     *   No ping. No TPS. No latency scaling. Pure wall clock.
     *   hurtTime transitions during pause are ignored — burst only opens
     *   after the pause expires naturally.
     *
     * Tick alignment:
     *   Only one attack packet per currentTime/50 tick bucket.
     *
    ...
    private boolean isHitSelectPaused() {
        ...
        // Fresh target: reset state, capture initial hurtTime baseline
        if (this.freshTarget) {
        ...
        // Detect hurtTime→0 transition on this call
        boolean hurtTimeCleared = this.prevHurtTime > 0 && curHurtTime == 0;

        switch (this.burstPhase) {

            case IDLE: {
                // Wait for entity to become hittable.
                // If hurtTime is already 0 on engagement, open immediately.
                boolean canOpen = hurtTimeCleared || curHurtTime == 0;
                if (!canOpen) {
                    this.prevHurtTime = curHurtTime;
                    return true;
                }
            ...
            case BURST: {
                ...
                long currentTick = now / 50L;
                if (currentTick == this.lastAttackTick) {
                    this.prevHurtTime = curHurtTime;
                    return true;
                }

                // ── APS interval gate ─────────────────────────────────────
                if (now - this.lastClickAttempt < this.nextClickInterval) {
                    this.prevHurtTime = curHurtTime;
                    return true;
                }

                // ── All conditions pass ───────────────────────────────────
                // ✓ pause inactive  ✓ burst active  ✓ APS elapsed
                // ✓ new tick        ✓ target valid
                this.prevHurtTime = curHurtTime;
                return false;
            }

            case PAUSE: {
                // ── Pure time-based pause ─────────────────────────────────
                // hurtTime transitions during pause are intentionally ignored.
                if (now - this.lastBurstEnd < this.midTradePause.getValue()) {
                    this.prevHurtTime = curHurtTime;
                    return true;
                }

                // Pause expired — check hurtTime before reopening.
                // If still in i-frames, drop to IDLE and wait for transition.
                if (curHurtTime > 0) {
                    this.burstPhase   = BurstPhase.IDLE;
                    this.prevHurtTime = curHurtTime;
                    return true;
                ...
                // Pause expired AND entity hittable → open burst
                this.burstPhase        = BurstPhase.BURST;
                this.burstWindowStart  = now;
                this.lastClickAttempt  = now - this.nextClickInterval;
                this.nextClickInterval = this.randomClickInterval();
                BackTrack bt = (BackTrack) Myau.moduleManager.getModule(BackTrack.class);
                if (bt != null && bt.isEnabled()) bt.onKillAuraResuming();
                this.prevHurtTime = curHurtTime;
    ...
    private void armHitSelect() {
        ...
        this.nextClickInterval = this.randomClickInterval();
        // Stamp the current 50ms tick so no second packet goes out this tick
        this.lastAttackTick    = now / 50L;
    }
